...
import { generateAccountResponse } from "./bedrock-operations.mjs";
...
export const lambdaHandler = async (event) => {
    ...
    try {
        ...
        // Generate AI account response using Bedrock
        const accountResponse = await generateAccountResponse(emailJson, emailData);
        ...
        // Format email data for display with Bedrock response at top and account inquiry details at bottom
...
<div style="font-family: Arial, sans-serif; line-height: 1.6;">
    <div style="background-color: #fff3cd; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h2 style="color: #2c3e50; margin-top: 0;">Account Response</h2>
        <div style="white-space: pre-wrap;">${accountResponse}</div>
    </div>
    ...
    <div style="background-color: #f1f3f4; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h3 style="color: #495057; margin-top: 0;">Account Inquiry Details</h3>
...
`;
        ...
        if (sendEmail) {
            try {
                ...
                // to include a custom reply-to address like accounts@yourdomain.com
                // or include data in the from email address like an account reference
                // such as accounts_844443985@yourdomain.com where replies would 
                // include a key to context.

                // Generate a dynamic account reference based on message ID
                const accountReference = emailData.messageId ? 
                    emailData.messageId.substring(0, 8) : 
                    Date.now().toString().substring(5);

                let emailSubject = `Re: ${emailData.originalEmail?.subject || 'Account Inquiry'} [Ref: ${accountReference}]`;
                // Optionally set the email subject programmatically
                // Currently it is just a generic response to the sender.
                // But it can include data in the subject like an account reference
                // such as "Your Account Inquiry ,<844443985>"
                // Also use "RE: " prefix to indicate a reply to the sender and
                // establish threading that is applicable to many Mail Clients.

                let categories = ["account", "response"];
                ...
                let customArgs = {
                    ...
                    "accountReferenceNumber": accountReference,
                    "category": emailData.categorization?.category || 'account',
                ...
                const publishCommand = new PublishCommand({
                    ...
                    Subject: "Account Response Email"
                });
        ...
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: sendEmail ? 
                    "Account email processed successfully and published to SNS" : 
                    "Account email processed successfully (no email sent)",
                category: "account",
        ...
        return {
            ...
            body: JSON.stringify({
                success: false,
                message: "Failed to process account email",
                category: "account",
                error: error.message
            })
        };
