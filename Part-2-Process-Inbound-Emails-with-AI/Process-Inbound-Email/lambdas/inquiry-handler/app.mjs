...
import { generateInquiryResponse } from "./bedrock-operations.mjs";
...
export const lambdaHandler = async (event) => {
    ...
    try {
        ...
        // Generate AI inquiry response using Bedrock
        const inquiryResponse = await generateInquiryResponse(emailJson, emailData);
...
<div style="font-family: Arial, sans-serif; line-height: 1.6;">
    <div style="background-color: #f0f8ff; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h2 style="color: #2c3e50; margin-top: 0;">General Inquiry Response</h2>
        <div style="white-space: pre-wrap;">${inquiryResponse}</div>
    </div>
    ...
    <div style="background-color: #f1f3f4; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        <h3 style="color: #495057; margin-top: 0;">General Inquiry Details</h3>
...
`;
        ...
        if (sendEmail) {
            try {
                ...
                // to include a custom reply-to address like info@yourdomain.com
                // or include data in the from email address like a reference number
                // such as info_844443985@yourdomain.com where replies would 
                // include a key to context.

                // Generate a dynamic reference number based on message ID
                const referenceNumber = emailData.messageId ? 
                    emailData.messageId.substring(0, 8) : 
                    Date.now().toString().substring(5);

                let emailSubject = `Re: ${emailData.originalEmail?.subject || 'General Inquiry'} [Ref: ${referenceNumber}]`;
                // Optionally set the email subject programmatically
                // Currently it is just a generic response to the sender.
                // But it can include data in the subject like a reference number
                // such as "Your Inquiry Reference ,<844443985>"
                // Also use "RE: " prefix to indicate a reply to the sender and
                // establish threading that is applicable to many Mail Clients.

                let categories = ["inquiry", "response"];
                ...
                let customArgs = {
                    ...
                    "category": emailData.categorization?.category || 'inquiry',
                ...
                const publishCommand = new PublishCommand({
                    ...
                    Subject: "General Inquiry Response Email"
                });
        ...
        return {
            ...
            body: JSON.stringify({
                success: true,
                message: sendEmail ? 
                    "Inquiry email processed successfully and published to SNS" : 
                    "Inquiry email processed successfully (no email sent)",
                category: "inquiry",
        ...
        return {
            ...
            body: JSON.stringify({
                success: false,
                message: "Failed to process inquiry email",
                category: "inquiry",
                error: error.message
            })
        };
